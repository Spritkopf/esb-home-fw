 ...
 * \param[in] chan_id           ID of the channel (0 < chan_id < BINARY_SENSOR_CHAN_NUM)
 * \param[out] p_value          pointer to buffer to store the current value of the channel (CHAN_VAL_FALSE (0) | CHAN_VAL_TRUE(1))
 * \retval ESB_PROT_ERR_OK      OK
 * \retval ESB_PROT_ERR_PARAM   invalid channel ID or NULL pointer
 */
 ...
 * \retval ESB_PROT_ERR_OK      OK
 * \retval ESB_PROT_ERR_INIT    Module is not initialized, call ::binary_sensor_init and ::binary_sensor_set_central_address first
 */
