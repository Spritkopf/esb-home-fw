...
typedef enum {
    ...
    SIMPLE = 0x10,    /*!< Device only has one single light source, Only supports setting brightness value*/
} light_capability_flags_t;

typedef enum {
    LIGHT_PROP_STATE      = 0x00,  /*!< Describes if the device is ON or OFF, data: [state] */
    LIGHT_PROP_BRIGHTNESS = 0x01,  /*!< Describes the brightness, data: [brightness] */
    LIGHT_PROP_RGB        = 0x02,  /*!< Describes the RGB value, data: [r, g, b] */
    LIGHT_PROP_RGBW       = 0x03,  /*!< Describes the RGBW value, data: [r, g, b, w] */
    LIGHT_PROP_TEMP       = 0x04,  /*!< Describes color temperature, data: [kelvin] */
    LIGHT_PROP_HSI        = 0x05,  /*!< Describes the HSI value, data: [h, s, i] */
 ...
 * themselves
 */
typedef esb_protocol_msg_err_t (*light_cmd_callback)(light_property_id_t prop_id, light_property_t* p_prop_data);

/*!
 * \brief Initialize the "ESB Light" application layer
 * \details Upon initialization the user application can specify which light property actually are supported
 ...
 */
esb_protocol_err_t esb_light_init(const uint8_t peripheral_address[5], light_capability_flags_t capabilities);
 ...
 * \param[in] prop_id           ID of the property, see ::light_property_id_t
 * \param[in] p_prop_data       Pointer to data of the property see ::light_property_t
 ...
 * \param[in] prop_id           ID of the property, see ::light_property_id_t
 * \param[out] p_prop_data      Pointer to buffer to store property data, see ::light_property_t
 * \retval ESB_PROT_ERR_OK      OK
 * \retval ESB_PROT_ERR_VALUE   unknown property ID
 * \retval ESB_PROT_ERR_PARAM  illegal parameter (NULL-pointer)
 */
 ...
 * \param[in] prop_id           ID of the property to report, see ::light_property_id_t
 * \param[in] p_prop_data       Pointer to data of the property see ::light_property_t
 * \retval ESB_PROT_ERR_OK      OK
 * \retval ESB_PROT_ERR_VALUE   unknown property ID
